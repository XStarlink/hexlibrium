...
class NPC65{
	...
	set action(name){
		...
		if (clip!==undefined){
			...
			action.reset();
			const nofade = this.actionName == 'ThumbsUp'; ////shot
	...
	update(dt){
        ...
        if (this.calculatedPath && this.calculatedPath.length) {
            ...
            let pathLegComplete = (vel.lengthSq()<0.01); //0.01
            ...
            if (!pathLegComplete) {
                ...
                if (this.quaternion) player.quaternion.slerp(this.quaternion, 0.1); //0.1

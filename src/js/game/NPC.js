...
class NPC{
	...
	initSounds(){
		...
		this.sfx.load('bassSwitch', true, 0.6, this.object);
	...
	newPath(pt){
        ...
        if (this.pathfinder===undefined){
            ...
            this.action = 'Walking'; //walking
            return;
        }
        
		if (this.sfx) this.sfx.play('bassSwitch');
		...
		if (this.calculatedPath && this.calculatedPath.length) {
			...
			if (this.sfx) this.sfx.stop('bassSwitch');
	...
	set action(name){
		...
		if (clip!==undefined){
			...
			if (name=='shot'){ //shot
				...
				this.dead = true;
				if (this.sfx){
					this.sfx.stop('bassSwitch');
					this.sfx.play('groan');
				}
				delete this.calculatedPath;
			}
			action.reset();
			const nofade = this.actionName == 'shot'; //shot
	...
	set firing(mode){
		this.isFiring = mode;
		if (mode){
			this.action = "ThumbsUp"; //firingwalk
	...
	update(dt){
		const speed = (this.actionName=='Wave') ? this.speed*0.6 : this.speed; //firingwalk
		const player = this.object;
			...
			if (!this.aggro){
				...
				if (this.withinAggroRange()){
					...
					if (!this.isFiring){
						...
						// if (this.isFiring && this.seeEnemy()){ //modified

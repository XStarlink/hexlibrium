...
class NPCHandler{
    constructor( game ){
		...
		this.ready = false;
		this.waypoints = game.waypoints; //added
		this.load();
		this.initMouseHandler(); //added
	}
	...
	initNPCs(gltf = this.gltf){
		...
		for(let i=0; i<2; i++) gltfs.push(this.cloneGLTF(gltf)); //number of npc
		...
		gltfs.forEach(gltf => {
			...
			object.traverse(function(child){
				if (child.isMesh){
					...
					if (child.name.includes('Thumb2.R_end')) rifle = child; // Rifle  Thumb2.R_end
				}
			});

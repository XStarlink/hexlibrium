...
class Screen{
	constructor(game, msg, size=1, oopt, bubble){
		...
		loader.load(
			...
			function ( texture ) {
				...
				self.mesh.material.transparent = false;
				self.mesh.material.needsUpdate = true;
				if (msg!==undefined) self.update(msg);
				// self.createOffscreenCanvas()
	...
	update(msg){
		...
		if (this.mesh.userData.context===undefined){
			...
			context.setAttribute('style', 'overflow: auto;', 'background: transparent') 
			context.style.cssText = 'position:absolute; top:1%; background:#FFFFFF; border:#444 solid medium; right:1%;'
			//iframeBlockly.src = 'http://localhost:3000/src/html/blockly.html'
			context.src = '/src/html/blockly.html'

			context.page = 
			context.className = 'iframeBlock'
